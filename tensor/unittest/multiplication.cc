...
{
  // dot -> 14+33+65 = 112
  typename TypeParam::Tensor tensor1 = libai::Tensor{  2,   3,   5 };
  typename TypeParam::Tensor tensor2 = libai::Tensor{  7,  11,  13 };

  typename TypeParam::Tensor result = libai::Matmul(tensor1, tensor2);
  EXPECT_EQ(result, libai::Tensor{ 112 });
}

TYPED_TEST_P(MultiplicationTestSuite, TensorMatmul)
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ { 3, 6, 9, 5 },
                                                     { 2, 8, 4, 7 },
                                                     { 5, 1, 7, 2 },
                                                     { 9, 3, 1, 5 } };
  typename TypeParam::Tensor tensor2 = libai::Tensor{ { 1, 8, 2 },
                                                     { 5, 3, 3 },
                                                     { 7, 4, 5 },
                                                     { 2, 9, 8 } };
...
TYPED_TEST_P(MultiplicationTestSuite, TensorMatmulContiguous)
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ { 3, 6, 9, 5 },
                                                     ...
                                                     { 5, 1, 7, 2 },
                                                     { 9, 3, 1, 5 } };
  typename TypeParam::Tensor tensor2 = libai::Tensor{ { 1, 5, 7, 2 },
                                                     { 8, 3, 4, 9 },
                                                     { 2, 3, 5, 8 } }
...
TYPED_TEST_P(MultiplicationTestSuite, TensorMatmulSemiContiguous)
{
  ...
  typename TypeParam::Tensor result =
    libai::Matmul(tensor1.Reshape(std::array{4UL,4UL},std::array{8L,1L}),
...
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ { 3, 0, 6, 0, 9, 0, 5, 0 },
                                                     { 0, 0, 0, 0, 0, 0, 0, 0 },
                                                     { 2, 0, 8, 0, 4, 0, 7, 0 },
                                                     { 0, 0, 0, 0, 0, 0, 0, 0 },
                                                     { 5, 0, 1, 0, 7, 0, 2, 0 },
                                                     ...
                                                     { 9, 0, 3, 0, 1, 0, 5, 0 },
                                                     { 0, 0, 0, 0, 0, 0, 0, 0 } };
  typename TypeParam::Tensor tensor2 = libai::Tensor{ { 1, 0, 5, 0, 7, 0, 2, 0 },
                                                     { 0, 0, 0, 0, 0, 0, 0, 0 },
                                                     { 8, 0, 3, 0, 4, 0, 9, 0 },
                                                     { 0, 0, 0, 0, 0, 0, 0, 0 },
                                                     { 2, 0, 3, 0, 5, 0, 8, 0 },
  ...
  typename TypeParam::Tensor result =
    libai::Matmul(tensor1.Reshape(std::array{4UL,4UL},std::array{16L,2L}),
...
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ { 3, 2, 5, 3 },
                                                     { 6, 8, 1, 7 },
                                                     { 9, 4, 7, 2 } };
  typename TypeParam::Tensor tensor2 = libai::Tensor{ 1, 5, 7, 2 };

  libai::Tensor expected{ 3 * 1 + 2 * 5 + 5 * 7 + 3 * 2,   // 54
                         6 * 1 + 8 * 5 + 1 * 7 + 7 * 2,   // 67
                         9 * 1 + 4 * 5 + 7 * 7 + 2 * 2 }; // 82
...
TYPED_TEST_P(MultiplicationTestSuite, TensorMatVecSemiContiguous)
{
  ...
  typename TypeParam::Tensor result =
    libai::Matmul(tensor1.Reshape(std::array{3UL,4UL},std::array{8L,1L}), tensor2);
...
TYPED_TEST_P(MultiplicationTestSuite, TensorMatVecNonContiguous)
{
  ...
  typename TypeParam::Tensor result =
    libai::Matmul(tensor1.Reshape(std::array{3UL,4UL},std::array{8L,1L}),
...
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ 1, 5, 7, 9 };
  typename TypeParam::Tensor tensor2 = libai::Tensor{ { 3, 2, 5 },
                                                     { 6, 8, 6 },
                                                     { 9, 4, 7 },
                                                     { 3, 7, 2 } };

  libai::Tensor expected{ 1 * 3 + 5 * 6 + 7 * 9 + 9 * 3,   // 123
                         1 * 2 + 5 * 8 + 7 * 4 + 9 * 7,   // 133
                         1 * 5 + 5 * 6 + 7 * 7 + 9 * 2 }; // 102
...
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ 1, 5, 7, 9 };
  typename TypeParam::Tensor tensor2 = libai::Tensor{ { 3, 6, 9, 3 },
                                                     { 2, 8, 4, 7 },
                                                     { 5, 6, 7, 2 } };
  ...
  typename TypeParam::Tensor result =
    libai::Matmul(tensor1, tensor2.Reshape(std::array{4UL,3UL},std::array{1L,4L}));
...
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ { 3, 6, 9 },
                                                     { 0, 0, 0 },
                                                     { 2, 8, 4 },
                                                     { 0, 0, 0 },
                                                     { 5, 1, 7 },
                                                     { 0, 0, 0 } };
  typename TypeParam::Tensor tensor2 = libai::Tensor{ 1, 5, 7 };
  libai::Tensor expected{ 3 * 1 + 6 * 5 + 9 * 7,   // 96
                         2 * 1 + 8 * 5 + 4 * 7,   // 70
                         5 * 1 + 1 * 5 + 7 * 7 }; // 59

  typename TypeParam::Tensor result =
    libai::Matmul(tensor2,
...
TYPED_TEST_P(MultiplicationTestSuite, TensorVecMatNonContiguous)
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ { 3, 6, 9 },
                                                     ...
                                                     { 5, 1, 7 },
                                                     { 0, 0, 0 } };
  typename TypeParam::Tensor tensor2 = libai::Tensor{ 1, 0, 5, 0, 7, 0 };
  libai::Tensor expected{ 3 * 1 + 6 * 5 + 9 * 7,   // 96
                         2 * 1 + 8 * 5 + 4 * 7,   // 70
                         5 * 1 + 1 * 5 + 7 * 7 }; // 59

  typename TypeParam::Tensor result =
    libai::Matmul(tensor2.Reshape(std::array{3UL},std::array{2L}),
                 tensor1.Reshape(std::array{3UL,3UL},std::array{1L,6L}));
  EXPECT_EQ(result, expected);
}
...
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ { 4.4f, 6.6f, 8.8f }, { 7.7f, 5.5f, 3.3f } };
  typename TypeParam::Tensor tensor2 = libai::Tensor{ 1.0f/11.f };
  libai::Tensor expected{ { 0.4f, 0.6f, 0.8f }, { 0.7f, 0.5f, 0.3f } };
...
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ 1.0f/11.f };
  typename TypeParam::Tensor tensor2 = libai::Tensor{ { 4.4f, 6.6f, 8.8f }, { 7.7f, 5.5f, 3.3f } };
  libai::Tensor expected{ { 0.4f, 0.6f, 0.8f }, { 0.7f, 0.5f, 0.3f } };
...
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ 3, 6, 9, 2, 8, 4 };
  typename TypeParam::Tensor tensor2 = libai::Tensor{ 1, 8, 5, 3, 7, 4 };
  libai::Tensor expected{ 3,48,45, 6,56,16 };

  typename TypeParam::Tensor result = libai::Mul(tensor1, tensor2);
  EXPECT_EQ(result, expected);
}
...
{
  typename TypeParam::Tensor tensor1 = libai::Tensor{ { 3, 6, 9 }, { 2, 8, 4 }};
  typename TypeParam::Tensor tensor2 = libai::Tensor{ { 1, 8, 5 } };
  libai::Tensor expected{ { 3,48,45 }, { 2,64,20 }};
...
{
  auto random1 = libai::Random<libai::Tensor, float>({10000,7000})();
  auto random2 = libai::Random<libai::Tensor, float>({10000,7000})();

  typename TypeParam::Tensor tensor1{random1};
  typename TypeParam::Tensor tensor2{random2};
  typename TypeParam::Tensor result = libai::Mul(tensor1, tensor2);

  libai::Tensor expected = libai::Mul(random1, random2);
  EXPECT_EQ(result, expected);
}
